...
const myEncode = (str, privatekey) => {
  const privateString = privatekey
    .split('')
    .reverse()
    .map(e => e.charCodeAt())
    .join('');
  ...
  const resString = str
    .split('')
    .reverse()
    .map(e => e.charCodeAt() + privateString)
    .join('-');
